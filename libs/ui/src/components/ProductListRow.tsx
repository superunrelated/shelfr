...
export function ProductListRow({
  title, shopName, price, imageUrl, status, rating,
  archived = false, selected = false,
  ...
  return (
    ...
    >
      {compareMode && (
        <div className={`ml-4 w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${compareSelected ? 'bg-[#1c1e2a] text-white' : 'border border-neutral-300 text-transparent'}`}>
          <RiCheckLine size={12} />
        </div>
      )}
      ...
      <div className="flex-1 py-3 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <p className="text-[13px] font-medium text-[#1c1e2a] truncate font-serif">{title}</p>
          <Badge status={status} showLabel />
        </div>
        <p className="text-[10px] text-neutral-400 uppercase tracking-wider">{shopName}</p>
      </div>
      ...
      <div className="w-24 flex-shrink-0 text-right">
        <span className={`text-[15px] font-semibold ${archived ? 'text-neutral-400 line-through' : 'text-[#1c1e2a]'}`}>
